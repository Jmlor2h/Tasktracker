...
const TodoTask = require("./models/collectdata"); // sets up collections for data to mongodb
dotenv.config();
...
app.get("/", (req, res) => {
  ...
  res.render("home.ejs", { todoTasks: tasks }); // todotasks: tasks allows data to be read from clientside. 
  console.log('loaded');
  });
  });
...
app.post('/',async (req, res) => {
  const todoTask = new TodoTask({ // object/constructor used for data
...
TodoTask.find({}, (err, tasks) => {
res.render("edittask.ejs", { todoTasks: tasks, idTask: id }); // sets values and allows clientside to read/write data through ejs
});
})
.post((req, res) => {
...
TodoTask.findByIdAndUpdate(id, { content: req.body.content }, err => { // after finding id, updates content with user input
...
app.route("/remove/:id").get((req, res) => {
  const id = req.params.id; // sets a const of id to the id of the request(which ever task you clicked on that activated this method).
  TodoTask.findByIdAndRemove(id, err => { // finds id and deletes.
  if (err) return res.send(500, err);
  res.redirect("/");
  });
  });